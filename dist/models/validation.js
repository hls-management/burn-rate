 ...
 */
export function validateGamePhase(phase, turn) {
    const errors = [];
 ...
 */
export function validateStateTransition(previousState, newState) {
    const errors = [];
 ...
 */
export function createInitialGameState(playerState, aiState) {
    const gameState = {
        turn: 1,
        player: playerState,
        ai: aiState,
 ...
 */
export function determineGamePhase(turn) {
    if (turn <= 5)
        return 'early';
    if (turn <= 15)
        return 'mid';
